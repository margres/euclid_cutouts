"""
Combine the per-pipeline lens mosaics into a single grid: one lens per row,
one pipeline per column. Reuses the cutouts already written by
make_lens_mosaics.py under results/lens_mosaics_r2/.

Run:
    python make_combined_lens_mosaic.py
"""
...
THUMB      = 120
LABEL_W    = 175   # left column for lens names
HEADER_H   = 30
...
def main():
    sample = pd.read_csv(os.path.join(OUTPUT_DIR, "lens_sample.csv"))
    n = len(sample)
    ...
    for r, (_, row) in enumerate(sample.iterrows()):
        ...
        for c, (_, _, path_fn) in enumerate(PIPELINES):
            ...
            try:
                ...
                # missing/failed cutout -- leave black
                pass

    out_path = os.path.join(OUTPUT_DIR, "mosaic_combined_all_pipelines.png")
    mosaic.save(out_path)
    logging.info(f"Saved combined mosaic: {out_path} ({n} lenses x {n_cols} pipelines, {width}x{height}px)")
